...
```mermaid
flowchart TD
    A[Input Stream: EHR Inbox] --> B[Classifier Agent]
    B --> C[Draft Agent]
    C --> D[PRISM Critic]
...
```mermaid
flowchart TD
    T[Trigger: Discharge Order Signed] --> R[Reconciliation Agent]
    R --> S[Scheduling Agent]
    S --> H[Home Health Agent]
    H --> E[Education Agent]
    E --> RS[Risk Scoring Agent]
    RS --> SU[Summary Agent]
    SU --> C[Clinician Approval Gate]
```
...
```mermaid
flowchart TD
    A[Trigger: SIEM Alert P1] --> O[Orchestrator]
    O --> TC[Threat Classifier]
    O --> AI[Asset Inventory]
    O --> CVE[CVE Lookup]
    O --> HC[Historical Correlator]
    TC --> SY[Synthesis Agent]
    AI --> SY
    CVE --> SY
    HC --> SY
    SY --> PC[PRISM Critic]
    PC --> AR[Analyst Review Queue]
```
...
```mermaid
flowchart TD
    F[Continuous Data Feed] --> M[Monitor Agent]
    M --> R[Risk Scoring Agent]
    R --> P[Playbook Router]
    P -->|Low Severity| L[Automated Report Agent]
    P -->|Med Severity| D[Draft Action + Human Approval]
    P -->|High Severity| E[Immediate Escalation + Circuit Breaker]
```
...
```mermaid
flowchart TD
    PM[PM Input: 3-sentence vibe] --> RE[Researcher Agent]
    RE --> WR[Writer Agent: EDGE + PRISM PRD]
    WR --> CR[Critic Agent: PRISM Guardrails]
    CR -->|Approved| RV[PM Review + Ship]
    CR -->|Revisions Required| WR
```
...
| Dimension | Benefit | Risk | Mitigation |
|---|---|---|---|
| Quality | Consistent structure; PRISM guardrails enforced by default | Generic output if the input vibe is too vague | Require 3–5 context sentences minimum; front-load domain specifics |
